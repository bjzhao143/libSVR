...
class GridOption:
    def __init__(self, dataset_pathname, options):
        ...
        if sys.platform != 'win32':
            ...
            # example for windows
            self.svmtrain_pathname = os.path.join(dirname, r'c:\libsvm-3.22\windows\svm-train.exe')
...
def gridgregression(filename):
    dataset_pathname = filename
    options = r'-log2c -10,10,1 -log2g -10,10,1 -log2p -10,10,1 -v 10 -s 3 -t 2'
    try:
        return find_parameters(dataset_pathname, options)
    except (IOError, ValueError) as e:
        sys.stderr.write(str(e) + r'\n')
